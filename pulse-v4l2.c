...
)
{
	...
	RECT roi_face;
	int nd=0;
	pr_debug(DSP_INFO,"%s: p=%p, %d, %d, fm=%.1f, fM=%.1f\n", __func__, p, width, height, band_min, band_max);

	if(V4L2_PIX_FMT_MJPEG == sfmt){
#if 0
		IplImage* frame;
		CvMat cvmat = cvMat(height, width, CV_8UC3, (void*)p);//MJPEG buffer p
		...
		//if(enableInfraRed)
		//	infrared_to_rgb888(width,height, (unsigned char *)p, imagedata_rgb);
		//else
		{
			imagedata_rgb=(unsigned char *)malloc(width * height * 3);//RGB888
			imagedata_Y=(unsigned char *)malloc(width * height);
			yuyv_to_rgb24(width, height, (unsigned char *)p, imagedata_rgb);
			extractY(width, height, (unsigned char *)p, imagedata_Y);
		}
		...
		//pr_debug(DSP_INFO,"mean_roi(%.4f,%.4f,%.4f)\n", mean_roi[0],mean_roi[1],mean_roi[2]);
		if(imagedata_rgb)
			free(imagedata_rgb);
		if(imagedata_Y)
			free(imagedata_Y);
...
static CvScalar processFrame_cv(const void *p, int size)
{
	...
	float pr=0.0f;
	int nface=0;
	pr_debug(DSP_INFO,">>>>>%s: p=%p, w=%d, h=%d\n", __func__, p, win_width, win_height);
	...
	cvSetData(gray, imagedata_Y, win_width);

	void *imagedata_rgb=malloc(win_width * win_height * 3);//RGB888
	yuyv_to_rgb24(win_width, win_height, (unsigned char *)p, imagedata_rgb);
	IplImage* framecopy = cvCreateImageHeader(cvSize(win_width, win_height), IPL_DEPTH_8U, 3);
	cvSetData(framecopy, imagedata_rgb, win_width*3);
	...
	sprintf(tempstr,"%.1fbpm (%.2fHz)", pr, pr/60.0f);
	...
	if(framecopy)
	  cvReleaseImageHeader(&framecopy);
	if(gray)
